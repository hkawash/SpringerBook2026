...
\begin{equation}
    \bm{d}_i(t)=w_{ii}(t)\,\tilde{\bm{d}}_i(t), \qquad \|\tilde{\bm{d}}_i(t)\|=1,
    \label{eq:continuous_time_autonomous}
\end{equation}
where $\tilde{\bm{d}}_i(t)$ indicates $i$'s preferred direction and $w_{ii}(t)\!\ge\!0$ represents its strength.
...
Let $\tau = \tau_d T_o$ with $\tau_d \in \mathbb{Z}_{>0}$, and define $\bm{p}_{ij,t} = \bm{p}_{j,t} - \bm{p}_{i,t}$ and $\tilde{\bm{p}}_{ij,t} = \bm{p}_{ij,t}/\|\bm{p}_{ij,t}\|$. 
Then, for $t$ in window $k$,
\begin{equation}
    \bm{v}_{i,t+\tau_d}
    = \sum_{j\ne i} w_{ij,k}\,\tilde{\bm{p}}_{ij,t}
      + \bm{d}_{i,k}
...
\begin{align}
  ...
  &X_t=
  \begin{bmatrix}
    \tilde{\bm{p}}_{i1,t} & \cdots & \tilde{\bm{p}}_{i,i-1,t} &
    \tilde{\bm{p}}_{i,i+1,t} & \cdots & \tilde{\bm{p}}_{iN,t} & I_d
  \end{bmatrix},\\
  &X=
  \begin{bmatrix}
    X_{b_k}\\ \vdots\\ X_{e_k}
...
\begin{equation}
    \bm{v}_{i,t+\tau_d}
    = \sum_{j\ne i} w_{ij,k}^{\mathrm{att}}\,\tilde{\bm{p}}_{ij,t}
      + \sum_{j\ne i, j\in \mathcal{N}^{\mathrm{ali}}_{i,t}} w_{ij,k}^{\mathrm{ali}}\,\tilde{\bm{v}}_{j,t}
      + \bm{d}_{i,k}
      + \bm{\epsilon}_{i,t+\tau_d},
    \label{eq:discrete_with_alignment_model}
\end{equation}
where $\tilde{\bm{v}}_{j,t}=\bm{v}_{j,t}/\|\bm{v}_{j,t}\|$ is the normalized velocity of individual $j$.
...
Experiments were conducted over three days with three trials per day, testing four conditions: control (C), slow (S1: $0.286~^\circ/\mathrm{s}$), medium (S2: $0.572~^\circ/\mathrm{s}$), and fast rotation (S3: $1.144~^\circ/\mathrm{s}$).
...
Letting $\tilde{\bm{s}}_{i,t}$ denote the normalized direction of the visual stimulus at the position of fish $i$ at time $t$, the model is expressed as 
\begin{equation}
    \bm{v}_{i,t+\tau_d}
    = \sum_{j\ne i} w_{ij,k}^{\mathrm{att}}\,\tilde{\bm{p}}_{ij,t}
      + w_{i,k}^{\mathrm{stim}}\,\tilde{\bm{s}}_{i,t}
      + \bm{d}_{i,k}
      + \bm{\epsilon}_{i,t+\tau_d},
...
Furthermore, to quantify the strength of the response to the external stimulus, we also define $StimCor$ as the mean correlation between the velocity of an individual and the clockwise rotational-direction vector at the position of $i$ with $\bm{p}_\mathrm{c}$ the center of rotation: $StimCor = \frac{1}{N} \sum_i \tilde{\bm{v}}_i \cdot \bm{r}_i$, 
% \begin{equation}
% StimCor = \frac{1}{N} \sum_i \tilde{\bm{v}}_i \cdot \bm{r}_i, 
% \end{equation}
where $\bm{r}_i$ is a unit vector in the clockwise rotational direction at $\bm{p}_i$ around $\bm{p}_\mathrm{c}$~\footnote{Since we use a left-handed coordinate system, which is standard in image coordinates, $\bm{r}_i = R (\bm{p}_i - \bm{p}_c)$ with $R$ the rotation matrix: $R (x, y)^\top = (-y, x)^\top$.}.
%  = R (\bm{p}_i - \bm{p}_c)$ with $R$ the rotation matrix: $R (x, y)^\top = (-y, x)^\top$.

Figure~\ref{fig:nn_velocity_correlation} shows an example of the NNCor and StimCor indices under the presentation of the rotational stimulus (S3: fast rotation).
...
However, the difference between the NNCor and StimCor values is small, and the relative contribution of internal coordination and external stimuli remain unclear. In the following paragraph, we introduce schooling indices derived from the extended interaction model to directly analyze these contributions.
% TODO: ここはもう少し検討
...
Likewise, $S_\mathrm{stim}$ represents the overall responsiveness of the group to the external visual stimulus, as it is derived from the total weight of the stimulus term. Here, we used $\tilde{\bm{s}}_i = \bm{r}_i$ for the stimulus direction in Eq.~\eqref{eq:discrete_with_stimulus_model}. 
...
\begin{figure}[t]
    ...
    \begin{minipage}[t]{0.45\textwidth}
        ...
        \begin{subfigure}[t]{\linewidth}
            ...
            \caption{Baseline indices: NNCor and StimCor}
...
For consistency, in the no-stimulus condition (C), the stimulus term $\tilde{\bm{s}}_i = \bm{r}_i$ in Eq.~\eqref{eq:discrete_with_stimulus_model} was retained to examine baseline tendency toward the rotational direction even in the absence of external stimuli.
